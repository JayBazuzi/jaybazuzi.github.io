...
| Code - Symbol | Stated Risk Level - Meaning         |
|------|----------------------------------------------|
| `.`  | Proven Safe - Addresses all known and unknown risks.       |
| `^`  | Validated   - Addresses all known risks.                   |
| `!`  | Risky       - Known risks remain unverified.       |
| `@`  | (Probably, Could Be) Broken - No statement to risk attestation. |

It may be important to call out that the levels of risk attestation can be viewed to be on a scale, where the top `.` symbol in the chart is the least risky intended change, and the `@` symbol is of the highest risk (-> in the view of the codebase for introducing risk). 
